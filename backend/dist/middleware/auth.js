...
export const authMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (payload.kind === 'admin') {
            const admin = await runWithContext({ isAdmin: true }, (manager) => manager.getRepository(AdminGlobal).findOne({ where: { id: payload.id } }));
            if (!admin)
                return res.status(401).json({ message: 'Admin no encontrado' });
            req.auth = { admin, isAdminGlobal: true, tenantId: null };
            return next();
        }
        const user = await runWithContext({ isAdmin: true }, (manager) => manager.getRepository(User).findOne({ where: { id: payload.id } }));
        if (!user)
