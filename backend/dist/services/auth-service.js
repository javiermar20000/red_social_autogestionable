...
export const registerUser = async (email, password, nombre, role = RolUsuario.VISITANTE) => {
    if (!nombre) {
    ...
    if (role === 'admin') {
        const admin = await runWithContext({ isAdmin: true }, async (manager) => {
            ...
            if (exists)
                throw new Error('Correo ya registrado como admin');
...
export const loginUser = async (email, password) => {
    const admin = await runWithContext({ isAdmin: true }, (manager) => manager.getRepository(AdminGlobal).findOne({ where: { email, activo: true } }));
    if (admin) {
    ...
    const user = await runWithContext({ isAdmin: true }, (manager) => manager.getRepository(User).findOne({ where: { email, activo: true } }));
    if (!user)
        throw new Error('Credenciales inválidas');
